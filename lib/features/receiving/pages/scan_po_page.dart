...
class _ScanPoPageState extends State<ScanPoPage> {
  ...
  void _onScan() {
    ...
    if (barcode.isEmpty || _selectedPo == null) return;
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    const Text('Receive PO Session', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: AppColors.textPrimaryLight)),
                    IconButton(
                  ...
                  children: [
                    Expanded(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('SELECT PO #', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: AppColors.textSecondaryLight)),
                    ...
                    Expanded(
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: _selectedPo != null ? 'Scan barcode — press Enter' : 'Select a PO to begin scanning',
                          hintStyle: const TextStyle(color: AppColors.textSecondaryLight),
                          prefixIcon: const Icon(Icons.qr_code_scanner, color: AppColors.textSecondaryLight),
                          border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: AppColors.separatorColor)),
                          enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: AppColors.separatorColor)),
                          focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: AppColors.primary, width: 2)),
                          filled: true,
                          fillColor: _selectedPo != null ? AppColors.surfaceLight : AppColors.separatorColor,
                        ),
                      ),
                    ),
                    const SizedBox(width: 16),
                ...
                Row(
                  children: [
                    Expanded(child: _buildMetricCard('PROGRESS', '$_totalScanned / $_totalExpected', Icons.inventory_2, AppColors.info)),
                    const SizedBox(width: 16),
                    Expanded(child: _buildMetricCard('OVERAGES', '$_mismatches', Icons.warning, AppColors.warning)),
                    const SizedBox(width: 16),
                    Expanded(child: _buildMetricCard('UNEXPECTED', '$_unexpectedScans', Icons.cancel, AppColors.error)),
                  ],
                ),
                      ...
                      children: [
                        Padding(
                          ...
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              const Text('Consolidated Manifest', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.textPrimaryLight)),
                              Row(
                                children: [
                                  TextButton.icon(
                            ...
                            ? const Center(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    SizedBox(height: 16),
                                    Text('Select a PO to view manifest', style: TextStyle(color: AppColors.textSecondaryLight)),
                                  ],
                                ),
                              )
                            : ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return Row(
                                    children: [
                                      ...
                                      Expanded(
                                        child: Text(
                                          variance > 0 ? '+$variance (Over)' : variance < 0 ? '$variance (Short)' : 'Matched',
                                          style: TextStyle(
                                            color: variance > 0 ? AppColors.warning : variance < 0 ? AppColors.error : AppColors.success,
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          IconButton(
                                            icon: const Icon(Icons.remove_circle_outline, color: AppColors.textSecondaryLight),
                                          ...
                                          IconButton(
                                            icon: const Icon(Icons.add_circle_outline, color: AppColors.textSecondaryLight),
  ...
  Widget _buildMetaField(String label, String value, {bool isDate = false}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: AppColors.textSecondaryLight)),
        const SizedBox(height: 8),
        Container(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(value, style: const TextStyle(fontWeight: FontWeight.w500, color: AppColors.textPrimaryLight)),
              if (isDate && value != '---') const Icon(Icons.calendar_today, size: 16, color: AppColors.textSecondaryLight),
            ],
          ),
        ),
      ],
  ...
  Widget _buildMetricCard(String label, String value, IconData icon, Color color) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: AppColors.separatorColor),
        boxShadow: [
          BoxShadow(color: AppColors.textPrimaryLight.withValues(alpha: 0.02), blurRadius: 8, offset: const Offset(0, 2)),
        ],
      ),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
